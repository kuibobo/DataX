...
public class JSONReader {
    ...
    public static void readFromUrl(final String url,
        ...
        while (loop) {
            reqUrl = url + "?tableName=" + table + "&pageIndex=" + pageIndex + "&pageSize=" + pageSize;
            json = client.getJSONObject(reqUrl);
            jarr = json.getJSONObject("data").getJSONArray("list");
            columns = getListColumnEntry(readerSliceConfig, Key.COLUMN);

            for (int i = 0; i < jarr.size(); i++) {
                transportOneRecord(recordSender, columns, jarr.getJSONObject(i));
            }
            loop = columns.size() == pageSize;
            pageIndex++;
        }
    }
