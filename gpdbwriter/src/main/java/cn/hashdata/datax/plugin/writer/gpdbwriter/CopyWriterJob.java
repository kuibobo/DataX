...
public class CopyWriterJob extends CommonRdbmsWriter.Job {
	...
	public void prepare(Configuration originalConfig) {
		...
		for (String table : tables) {
			sqls.add("SELECT sc_truncate_error_log('" + table + "');");
	...
	public void post(Configuration originalConfig) {
		...
		for (String table : tables) {
			int errors = 0;
			ResultSet res = null;
			String sql = "SELECT count(*) from sc_read_error_log('" + table + "');";
			...
			if (errors > 0) {
				LOG.warn("加载表 {} 时发现 {} 条错误数据, 使用 \"SELECT * from sc_read_error_log('{}');\" 查看详情", table,
						errors, table);
			}
		}
