...
public class SingleTableSplitUtil {
        ...
        if (DATABASE_TYPE == DataBaseType.Oracle) {
            ...
            if (null == minMaxPK) {
                throw DataXException.asDataXException(DBUtilErrorCode.ILLEGAL_SPLIT_PK,
                        "根据切分主键切分表失败. App 仅支持切分主键为一个,并且类型为整数或者字符串类型. 请尝试使用其他的切分主键或者联系 DBA 进行处理.");
            }
            ...
            if (isStringType) {
                ...
                throw DataXException.asDataXException(DBUtilErrorCode.ILLEGAL_SPLIT_PK,
                        "您配置的切分主键(splitPk) 类型 App 不支持. App 仅支持切分主键为一个,并且类型为整数或者字符串类型. 请尝试使用其他的切分主键或者联系 DBA 进行处理.");
            }
        }
        String tempQuerySql;
    ...
    private static Pair<Object, Object> checkSplitPk(Connection conn, String pkRangeSQL, int fetchSize,  String table,
        ...
        try {
                ...
                if (isStringType(rsMetaData.getColumnType(1))) {
                    ...
                    while (DBUtil.asyncResultSetNext(rs)) {
                        ...
                        if (StringUtils.contains(minMax, '.')) {
                            throw DataXException.asDataXException(DBUtilErrorCode.ILLEGAL_SPLIT_PK,
                                    "您配置的App切分主键(splitPk)有误. 因为您配置的切分主键(splitPk) 类型 App 不支持. App 仅支持切分主键为一个,并且类型为整数或者字符串类型. 请尝试使用其他的切分主键或者联系 DBA 进行处理.");
                        }
                    }
                } else {
                    throw DataXException.asDataXException(DBUtilErrorCode.ILLEGAL_SPLIT_PK,
            ...
            throw DataXException.asDataXException(DBUtilErrorCode.ILLEGAL_SPLIT_PK, "App尝试切分表发生错误. 请检查您的配置并作出修改.", e);
        } finally {
        ...
        if (splitedRangeSize >= 2) {
            ...
            if (isLongType(splitedRange.get(0).getRight())) {
                ...
                throw DataXException
                        .asDataXException(
                                DBUtilErrorCode.ILLEGAL_SPLIT_PK,
                                "您配置的DataX切分主键(splitPk)有误. 因为您配置的切分主键(splitPk) 类型 App 不支持. App 仅支持切分主键为一个,并且类型为整数或者字符串类型. 请尝试使用其他的切分主键或者联系 DBA 进行处理.");
            }
        }
        return rangeSql;
    }
